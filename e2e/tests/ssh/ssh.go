...
var _ = DevPodDescribe("devpod ssh test suite", func() {
	ginkgo.Context("testing ssh command", ginkgo.Label("ssh"), ginkgo.Ordered, func() {
		...
		ginkgo.It("should start a new workspace with the platform and ssh into it", func() {
			tempDir, err := framework.CopyToTempDir("tests/ssh/testdata/local-test")
			framework.ExpectNoError(err)
			defer framework.CleanupTempDir(initialDir, tempDir)
			...
			err = f.DevPodProUp(devpodUpCtx, "testprossh", "git", "https://github.com/loft-sh/devpod-example-go")
			framework.ExpectNoError(err)

			devpodSSHDeadline := time.Now().Add(20 * time.Second)
			devpodSSHCtx, cancelSSH := context.WithDeadline(context.Background(), devpodSSHDeadline)
			defer cancelSSH()
			err = f.DevPodSSHEchoTestString(devpodSSHCtx, "testprossh")
			framework.ExpectNoError(err)
		})
		...
		ginkgo.It("should start a new workspace with the platform and forward gpg agent into it", func() {
			// skip windows for now
			if runtime.GOOS == "windows" {
				return
			}
			...
			devpodUpCtx, cancel := context.WithDeadline(context.Background(), devpodUpDeadline)
			defer cancel()
			err = f.DevPodProUp(devpodUpCtx, "gpgprotest", "git", "https://github.com/loft-sh/devpod-example-go", "--gpg-agent-forwarding")
			framework.ExpectNoError(err)
			...
			retries := 10
			for retries > 0 {
				err = f.DevPodSSHGpgTestKey(devpodSSHCtx, "gpgprotest")
				if err != nil {
					retries--
					time.Sleep(time.Second)
				} else {
		...
		ginkgo.It("should start a new workspace with the platform and forward a port into it", func() {
			// skip windows for now
			if runtime.GOOS == "windows" {
				return
			}
			...
			ginkgo.DeferCleanup(f.DevPodWorkspaceDelete, context.Background(), "testproportforward")
			...
			devpodUpCtx, cancel := context.WithDeadline(context.Background(), devpodUpDeadline)
			defer cancel()
			err = f.DevPodProUp(devpodUpCtx, "testproportforward", "image", "alpine:latest")
			framework.ExpectNoError(err)
			...
			cmd := exec.CommandContext(ctx, f.DevpodBinDir+"/"+f.DevpodBinName,
				[]string{
					"ssh", "testproportforward", "--command",
					"sh -c \"while true; do echo PONG | nc -n -lk -p " + strconv.Itoa(port) + "; done\"",
				}...,
			)
			err = cmd.Start()
			...
			go func() {
				...
				_ = f.DevpodPortTest(devpodSSHCtx, strconv.Itoa(port), "testproportforward")
